package com.binaryleaf.openweathercrudassignment.model

import android.arch.lifecycle.LiveData
import android.util.Log
import com.binaryleaf.openweathercrudassignment.network.NetworkRepository
...
class WeatherDataProcessor(val repository: WeatherDataRepository = WeatherDataRepository(), val networkRepository: NetworkRepository = NetworkRepository()) {
    ...
    fun getWeatherData(onWeatherDataListener: OnWeatherDataListener) {
        val call = networkRepository.weatherService.getWeather(networkRepository.id, networkRepository.appId, "metric", 24.8607, 67.0011)
